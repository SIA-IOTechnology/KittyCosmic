...
# Extension package root (…/kittycosmic/latest/src): templates, static, kittyos_cosmic
...
def _framework_root() -> str:
    """KittySploit project root: directory containing both ``lib/`` and ``core/`` (not this file's dir)."""
...
from core.models.models import Host, Vulnerability, Workflow, WorkflowExecution, Workspace
import json
...
# --- Flask app paths ---
# The KittyOS UI lives under kittyos_cosmic/
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
KITTYOS_TEMPLATES_DIR = os.path.join(BASE_DIR, 'kittyos_cosmic', 'templates')
KITTYOS_STATIC_DIR = os.path.join(BASE_DIR, 'kittyos_cosmic', 'static')

# Point Flask to the KittyOS template/static folders so routes like /os work.
app = Flask(
    __name__,
    template_folder=KITTYOS_TEMPLATES_DIR,
    static_folder=KITTYOS_STATIC_DIR,
    static_url_path='/static',
)

# Also allow other interface templates (e.g. KittyCollab) if needed.
_extra_template_dirs = [
    os.path.join(BASE_DIR, 'interfaces', 'kittycollab', 'templates'),
]
try:
    app.jinja_loader = ChoiceLoader(
        [app.jinja_loader] + [FileSystemLoader(p) for p in _extra_template_dirs if os.path.isdir(p)]
    )
except Exception:
    pass
...
from kittyos_cosmic.chat_server import ChatServerManager
chat_server_manager = None

# Proxy Manager for KittyProxy
from kittyos_cosmic.proxy_manager import ProxyManager
proxy_manager = None

# KittyCollab Server
collab_server = None
collab_server_thread = None
collab_server_port = 5006  # KittyCollab (separate from main UI)

# Main browser UI (KittyCosmic / KittyOS). Override with KITTYCOSMIC_HOST / KITTYCOSMIC_PORT.
KITTYOS_WEB_HOST = os.environ.get("KITTYCOSMIC_HOST", "127.0.0.1")
try:
    KITTYOS_WEB_PORT = int(os.environ.get("KITTYCOSMIC_PORT", "6223"))
except ValueError:
    KITTYOS_WEB_PORT = 6223
# Set KITTYCOSMIC_DEBUG=1 for Flask debug (noisier console).
KITTYOS_WEB_DEBUG = os.environ.get("KITTYCOSMIC_DEBUG", "").lower() in ("1", "true", "yes")
...
# Cache for listener options to avoid reloading modules
_listener_options_cache = {}
...
def init_framework():
    ...
    with framework_init_lock:
        ...
        try:
            print_info("Loading KittySploit (encryption, database, services)...")
            framework = Framework()
            ...
            if not framework.is_encryption_initialized():
                ...
                if not framework.initialize_encryption():
                    print_error("Failed to initialize encryption. Database will remain locked.")
            else:
            ...
            if SOCKETIO_AVAILABLE and socketio:
                from kittyos_cosmic.irc_bridge import WebIRCBridge
                irc_bridge = WebIRCBridge(socketio)
            
            # Initialize Chat Server Manager
            global chat_server_manager
            if socketio:
                chat_server_manager = ChatServerManager(socketio)
            ...
            print_success(
                "KittySploit ready (web terminal, IRC, chat, proxy, workflows)."
            )
            return framework
...
def serve_logo():
    """Serve logo image explicitly"""
    from flask import send_from_directory
    return send_from_directory(KITTYOS_STATIC_DIR, 'logo.jpg')

@app.route('/os')
def os_desktop_redirect():
    """Redirect /os to / for backward compatibility"""
    ...
    try:
        if framework and hasattr(framework, 'session_manager'):
            ...
            for session in all_sessions.get('standard', []):
                ...
                from core.session_manager import _make_json_serializable
                serializable_data = _make_json_serializable(session.data) if session.data else {}
    ...
    try:
        if framework and hasattr(framework, 'browser_server') and framework.browser_server:
            ...
            return jsonify({
                ...
                'stats': bs.stats,
                'obfuscate_js': getattr(bs, 'obfuscate_js', False),
                'obfuscation_level': getattr(bs, 'obfuscation_level', None) if getattr(bs, 'obfuscate_js', False) else None,
                'links': {
...
def browser_server_start():
    ...
    try:
        ...
        # Determine obfuscation settings
        obfuscate_js = obfuscation_level is not None and obfuscation_level != ''
        obfuscation_level_value = obfuscation_level if obfuscate_js else 'simple'
        ...
        if framework:
            ...
            framework.browser_server = BrowserServer(
                ...
                obfuscation_level=obfuscation_level_value
            )
...
def create_listener():
    ...
    try:
        ...
        port = data.get('port')
        host = data.get('host', '0.0.0.0')
        options = data.get('options', {})  # Additional options from the listener
        ...
        # Configure listener options - check for standard option attributes first
        # Some listeners use lhost/lport, others use rhost/rport, and some use host/port directly
        if hasattr(listener_instance, 'lhost'):
            if hasattr(listener_instance.lhost, 'value'):
                listener_instance.lhost.value = host
            else:
                listener_instance.lhost = host
        if hasattr(listener_instance, 'lport'):
            if hasattr(listener_instance.lport, 'value'):
                listener_instance.lport.value = int(port)
            else:
                listener_instance.lport = int(port)
        if hasattr(listener_instance, 'rhost'):
            if hasattr(listener_instance.rhost, 'value'):
                listener_instance.rhost.value = host
            else:
                listener_instance.rhost = host
        if hasattr(listener_instance, 'rport'):
            if hasattr(listener_instance.rport, 'value'):
                listener_instance.rport.value = int(port)
            else:
                listener_instance.rport = int(port)
        if hasattr(listener_instance, 'host'):
            if hasattr(listener_instance.host, 'value'):
                listener_instance.host.value = host
            else:
                listener_instance.host = host
        if hasattr(listener_instance, 'port'):
            if hasattr(listener_instance.port, 'value'):
                listener_instance.port.value = int(port)
            else:
                listener_instance.port = int(port)
        
        # Set additional options from the request
        for option_name, option_value in options.items():
            if hasattr(listener_instance, option_name):
                option_attr = getattr(listener_instance, option_name)
                # Check if it's an option object (OptString, OptPort, etc.) with a value attribute
                if hasattr(option_attr, 'value'):
                    # Set the value on the option object
                    try:
                        # Get the option type to determine how to set the value
                        option_type = type(option_attr).__name__
                        
                        if option_type in ['OptPort', 'OptInteger']:
                            option_attr.value = int(option_value) if option_value else 0
                        elif option_type == 'OptBool':
                            option_attr.value = bool(option_value) if isinstance(option_value, bool) else str(option_value).lower() in ['true', '1', 'yes', 'on']
                        else:
                            # OptString or other string-based options
                            option_attr.value = str(option_value) if option_value is not None else ''
                    except Exception as e:
                        print_error(f"Error setting option {option_name}: {e}")
                    ...
                    # Direct attribute assignment (not an option object)
                    try:
                        setattr(listener_instance, option_name, option_value)
                    except Exception as e:
                        print_error(f"Error setting attribute {option_name}: {e}")
        ...
        # Register as a Job so it appears in the Jobs UI
        try:
            from core.job_manager import global_job_manager
            # Store a back-reference so stop/delete can update the job status
            if not getattr(listener_instance, 'job_id', None):
                job_id = global_job_manager.add_job(
                    name=f"Listener: {listener_name}",
                    description=f"{module_path} @ {host}:{port}",
                    target=f"{host}:{port}",
                    module=listener_instance
                )
                if job_id:
                    listener_instance.job_id = job_id
        except Exception as e:
            # Non-fatal: listener can still run even if job manager fails
            print_error(f"Warning: failed to register listener job: {e}")
        
        return jsonify({
            'success': True,
            'listener_id': listener_id,
            'job_id': getattr(listener_instance, 'job_id', None),
...
@app.route('/api/listeners/info', methods=['GET'])
def get_listener_info():
    """Get listener module information and options"""
    module_path = request.args.get('module_path')
    if not module_path:
        return jsonify({'success': False, 'error': 'module_path parameter is required'}), 400
    try:
        if not framework:
            return jsonify({'success': False, 'error': 'Framework not initialized'}), 500
        
        # Normalize module path (handle both with and without .py extension)
        normalized_path = module_path
        if not normalized_path.endswith('.py'):
            # Get listener modules to check paths
            listener_modules = framework.module_loader.get_modules_by_type("listener")
            for path in listener_modules:
                if path == module_path or path == f"{module_path}.py" or path.endswith(f"/{module_path}.py"):
                    normalized_path = path
                    break
        
        # Check cache first
        with _listener_cache_lock:
            if normalized_path in _listener_options_cache:
                cached_data = _listener_options_cache[normalized_path]
                return jsonify({
                    'success': True,
        ...
        if normalized_path not in listener_modules:
            return jsonify({'success': False, 'error': f'Listener module not found: {module_path}'}), 404
        
        # Use the already loaded instance from get_modules_by_type instead of reloading
        # This is much faster as the module is already in memory
        listener_instance = listener_modules[normalized_path]
        
        # If the instance is not yet instantiated, load it
        # Check if it's a class or an instance
        if not hasattr(listener_instance, '__info__'):
            # It's a class, need to instantiate it
            try:
                import inspect
                sig = inspect.signature(listener_instance.__init__)
                if 'framework' in sig.parameters:
                    listener_instance = listener_instance(framework=framework)
                else:
                    listener_instance = listener_instance()
                if framework:
                    listener_instance.framework = framework
            except Exception as e:
                print_error(f"Error instantiating listener module {normalized_path}: {e}")
                # Fallback: try loading the module
                try:
                    listener_instance = framework.module_loader.load_module(normalized_path, framework=framework)
                except Exception as e2:
                    print_error(f"Error loading listener module {normalized_path}: {e2}")
                    import traceback
                    traceback.print_exc()
                    return jsonify({'success': False, 'error': f'Failed to load listener module: {str(e2)}'}), 500
        
        # Get module info and serialize non-JSON-serializable objects
        module_info_raw = getattr(listener_instance, '__info__', {})
        module_info = {}
        for key, value in module_info_raw.items():
            # Convert Handler and SessionType enums to strings
            if value is None:
                module_info[key] = None
            elif isinstance(value, (str, int, float, bool)):
                module_info[key] = value
            elif isinstance(value, dict):
                # Recursively convert dict values
                module_info[key] = {k: str(v) if not isinstance(v, (str, int, float, bool, list, dict, type(None))) else v for k, v in value.items()}
            elif isinstance(value, list):
                # Recursively convert list items
                module_info[key] = [str(item) if not isinstance(item, (str, int, float, bool, list, dict, type(None))) else item for item in value]
            else:
                # For enums and other objects, convert to string
                try:
                    # Try to get name attribute (for enums)
                    if hasattr(value, 'name'):
                        module_info[key] = value.name
                    else:
                        module_info[key] = str(value)
                except:
                    module_info[key] = str(value)
        
        # Get all options from the module
        # Options are defined at the class level as descriptors, so we need to check the class
        options = []
        seen_options = set()  # Track seen options to avoid duplicates
        
        # Get the class of the listener instance
        listener_class = type(listener_instance)
        
        # Check all attributes in the class and its base classes
        # Only check the direct class and immediate base classes to avoid checking too many
        classes_to_check = [listener_class]
        if hasattr(listener_class, '__bases__'):
            classes_to_check.extend([cls for cls in listener_class.__bases__ if cls != object])
        
        for base_class in classes_to_check:
            ...
            for attr_name in base_class.__dict__:
                ...
                if attr_name in seen_options:
                    continue
                
                try:
                    # Get the attribute from the class (not instance) to get the Option descriptor
                    attr = getattr(base_class, attr_name)
                    
                    # Skip if it's a method or callable (but not a descriptor)
                    if callable(attr) and not hasattr(attr, '__get__'):
                        continue
                    
                    # Check if it's an option (OptString, OptPort, OptInteger, etc.)
                    # Options are descriptors with 'description' and 'required' attributes
                    if hasattr(attr, 'description') and hasattr(attr, 'required'):
                        option_type = type(attr).__name__
                        
                        # Only include actual option types
                        if option_type.startswith('Opt'):
                            # Get the current value from the instance
                            current_value = getattr(listener_instance, attr_name, None)
                            
                            option_info = {
                                'name': attr_name,
                                'value': current_value if current_value is not None else attr._default_value if hasattr(attr, '_default_value') else '',
                                'description': attr.description if hasattr(attr, 'description') else attr_name,
                            ...
                            if hasattr(attr, '_default_value'):
                                option_info['default'] = attr._default_value
                            elif current_value is not None:
                                option_info['default'] = current_value
                            
                            # Skip only the standard bind/listen options (lhost, lport, rhost, rport)
                            # For FTP and similar listeners, 'host' and 'port' are connection options, not bind options
                            # So we include them as they are specific to the listener type
                            if attr_name in ['lhost', 'lport', 'rhost', 'rport']:
                                continue
                            
                            seen_options.add(attr_name)
        ...
        with _listener_cache_lock:
            _listener_options_cache[normalized_path] = {
                'info': module_info,
                'options': options
            }
        ...
        print_error(f"Error getting listener info: {e}")
        import traceback
        traceback.print_exc()
        return jsonify({'success': False, 'error': str(e)}), 500
    ...
    try:
        ...
        for listener_id, listener_instance in framework.active_listeners.items():
            try:
                ...
                # Try to get port from various possible attributes
                port = None
                # Check lport (for reverse listeners)
                if hasattr(listener_instance, 'lport'):
                    lport_val = listener_instance.lport
                    port = lport_val.value if hasattr(lport_val, 'value') else lport_val
                # Check rport (for bind listeners)
                if port is None and hasattr(listener_instance, 'rport'):
                    rport_val = listener_instance.rport
                    port = rport_val.value if hasattr(rport_val, 'value') else rport_val
                # Check port (for listeners like FTP that use direct port)
                if port is None and hasattr(listener_instance, 'port'):
                    port_val = listener_instance.port
                    port = port_val.value if hasattr(port_val, 'value') else port_val
                port = port or 0
                
                # Try to get host from various possible attributes
                host = None
                # Check lhost (for reverse listeners)
                if hasattr(listener_instance, 'lhost'):
                    lhost_val = listener_instance.lhost
                    host = lhost_val.value if hasattr(lhost_val, 'value') else lhost_val
                # Check rhost (for bind listeners)
                if host is None and hasattr(listener_instance, 'rhost'):
                    rhost_val = listener_instance.rhost
                    host = rhost_val.value if hasattr(rhost_val, 'value') else rhost_val
                # Check host (for listeners like FTP that use direct host)
                if host is None and hasattr(listener_instance, 'host'):
                    host_val = listener_instance.host
                    host = host_val.value if hasattr(host_val, 'value') else host_val
...
def start_listener(lid):
    ...
    try:
        ...
        if hasattr(listener, 'start'):
            ...
            if success:
                # Ensure it shows up in Jobs
                try:
                    from core.job_manager import global_job_manager
                    job_id = getattr(listener, 'job_id', None)
                    if job_id and global_job_manager.get_job(job_id):
                        global_job_manager.update_job_status(job_id, 'running', output='Listener started\n')
                    else:
                        info = getattr(listener, '__info__', {}) or {}
                        name = info.get('name', getattr(listener, 'name', 'Listener'))
                        host = getattr(listener, 'lhost', None) or getattr(listener, 'rhost', None) or '0.0.0.0'
                        port = getattr(listener, 'lport', None) or getattr(listener, 'rport', None) or 0
                        new_job_id = global_job_manager.add_job(
                            name=f"Listener: {name}",
                            description=f"{lid} @ {host}:{port}",
                            target=f"{host}:{port}",
                            module=listener
                        )
                        if new_job_id:
                            listener.job_id = new_job_id
                except Exception as e:
                    print_error(f"Warning: failed to update listener job on start: {e}")
        ...
        if hasattr(listener, 'stop'):
            ...
            if success:
                ...
                try:
                    ...
                    job_id = getattr(listener, 'job_id', None)
                    if job_id:
                        global_job_manager.update_job_status(job_id, 'completed', output='Listener stopped\n')
                except Exception as e:
                    print_error(f"Warning: failed to update listener job on stop: {e}")
...
def delete_listener(lid):
    ...
    try:
        ...
        # Mark job as killed/completed and detach module reference
        try:
            from core.job_manager import global_job_manager
            job_id = getattr(listener, 'job_id', None)
            if job_id:
                global_job_manager.update_job_status(job_id, 'killed', output='Listener deleted\n')
        except Exception as e:
            print_error(f"Warning: failed to update listener job on delete: {e}")
...
def run_module(sid):
    ...
    try:
        ...
        # Execute the module with output capture
        import io
        import sys
...
def guardian_status():
    ...
    try:
        ...
        gm = None
        if framework:
            # Guardian should already be initialized in Framework.__init__
            # Only create if it doesn't exist (shouldn't happen normally)
            if not hasattr(framework, 'guardian_manager') or framework.guardian_manager is None:
                try:
            ...
            if hasattr(framework, 'guardian_manager') and framework.guardian_manager:
        ...
        if gm:
            ...
            # Get recent operation history (last 20 operations)
            operation_history = []
            try:
                operation_history = gm.operation_history[-20:] if hasattr(gm, 'operation_history') and gm.operation_history else []
            except Exception as e:
                print_error(f"Error getting operation history: {e}")
    ...
    try:
        ...
        if framework:
            ...
            if hasattr(framework, 'guardian_manager') and framework.guardian_manager:
                ...
                print_info(f"[GUARDIAN] Enabled with verbose={verbose}, auto_action={auto_action}")
...
@app.route('/api/desktop-apps/preferences', methods=['GET'])
def get_desktop_apps_preferences():
    """Get desktop applications visibility preferences"""
    try:
        # Default list of all available desktop apps
        default_apps = {
            'terminal': {'name': 'Terminal', 'icon': 'terminal-outline', 'visible': True},
            'kittyproxy': {'name': 'KittyProxy', 'icon': 'shield-checkmark-outline', 'visible': True},
            'collab': {'name': 'KittyCollab', 'icon': 'code-slash-outline', 'visible': True},
            'workflows': {'name': 'Workflows', 'icon': 'git-branch-outline', 'visible': True},
            'module_launcher': {'name': 'Module Launcher', 'icon': 'rocket-outline', 'visible': True},
            'browser_server': {'name': 'Browser Server', 'icon': 'globe-outline', 'visible': True},
            'network_map': {'name': 'Network Map', 'icon': 'map-outline', 'visible': True},
            'sessions': {'name': 'Sessions', 'icon': 'people-outline', 'visible': True},
            'jobs': {'name': 'Jobs', 'icon': 'briefcase-outline', 'visible': True},
            'network': {'name': 'Hosts & Vuln', 'icon': 'bug-outline', 'visible': True},
            'ports': {'name': 'Ports', 'icon': 'server-outline', 'visible': True},
            'backdoor_generator': {'name': 'Backdoor Generator', 'icon': 'key-outline', 'visible': True},
            'web_delivery': {'name': 'Web Delivery', 'icon': 'cloud-download-outline', 'visible': True},
            'listeners': {'name': 'Listener Manager', 'icon': 'radio-outline', 'visible': True},
            'vnc': {'name': 'VNC Client', 'icon': 'desktop-outline', 'visible': True},
            'docker_environments': {'name': 'Docker', 'icon': 'cube-outline', 'visible': True},
            'chat_server': {'name': 'Chat Server', 'icon': 'chatbox-ellipses-outline', 'visible': True},
            'marketplace': {'name': 'Marketplace', 'icon': 'storefront-outline', 'visible': True},
            'notes': {'name': 'Notes', 'icon': 'document-text-outline', 'visible': True},
            'agent_launcher': {'name': 'Agent Launcher', 'icon': 'hardware-chip-outline', 'visible': True}
        }
        
        # Try to load saved preferences from a config file
        config_dir = os.path.join(os.path.expanduser("~"), ".kittysploit")
        prefs_file = os.path.join(config_dir, "desktop_apps_preferences.json")
        
        if os.path.exists(prefs_file):
            try:
                with open(prefs_file, 'r', encoding='utf-8') as f:
                    saved_prefs = json.load(f)
                    # Merge saved preferences with defaults (in case new apps were added)
                    for app_id, app_info in default_apps.items():
                        if app_id in saved_prefs:
                            default_apps[app_id]['visible'] = saved_prefs[app_id].get('visible', True)
                        else:
                            # New app, keep default visibility
                            pass
            except Exception as e:
                print_error(f"Error loading desktop apps preferences: {e}")
        ...
        import traceback
        print_error(f"Error getting desktop apps preferences: {e}")
        return jsonify({'error': str(e), 'traceback': traceback.format_exc()}), 500

@app.route('/api/desktop-apps/preferences', methods=['POST'])
def save_desktop_apps_preferences():
    """Save desktop applications visibility preferences"""
    try:
        data = request.json
        apps = data.get('apps', {})
        
        if not apps:
            return jsonify({'error': 'Apps data is required'}), 400
        ...
        return jsonify({
            'success': True,
            'message': 'Desktop apps preferences saved successfully'
        })
    except Exception as e:
        import traceback
        print_error(f"Error saving desktop apps preferences: {e}")
        return jsonify({'error': str(e), 'traceback': traceback.format_exc()}), 500
...
@app.route('/api/healt', methods=['GET'])  # backward/typo compatibility
def api_health():
    """Lightweight healthcheck used by KittyOS UI.

    Returns 200 when the HTTP backend is reachable.
    """
    try:
        import time as _time

        # Best-effort "ready" checks (should never throw).
        framework_ready = bool(globals().get('framework'))
        socketio_ready = bool(globals().get('socketio'))

        return jsonify({
            'ok': True,
            'framework_ready': framework_ready,
            'socketio_ready': socketio_ready,
            'timestamp': _time.time(),
        })
    except Exception as e:
        # Even on error, expose a structured response; keep status 500 for visibility.
        return jsonify({'ok': False, 'error': str(e)}), 500

@app.route('/api/system/stats', methods=['GET'])
def system_stats():
    """Fallback HTTP endpoint for realtime system stats (CPU/RAM/Disk/IP).

    The frontend normally uses Socket.IO events for this. When Socket.IO is
    unavailable/blocked, this endpoint keeps the widgets functional.
    """
    try:
        import psutil
        import socket
        ...
        # Windows-compatible root path (e.g. C:\)
...
def api_ports():
    """Fallback HTTP endpoint for local listening ports.

    The frontend normally uses Socket.IO events for this.
    """
    try:
        import psutil
        import socket
        ...
        return jsonify({'ports': ports_data})
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def terminal_exec():
    """Execute a command via REST API (fallback when SocketIO not available)"""
    try:
        ensure_terminal_backend()
        data = request.json or {}
        ...
        if not command:
            return jsonify({'success': False, 'error': 'Command is required', 'output': ''}), 400
        
        if not framework or not command_registry:
            return jsonify({'success': False, 'error': 'Backend not initialized', 'output': ''}), 500
        ...
        print_error(f"Error in terminal_exec API: {e}")
...
def list_workspaces():
    ...
    try:
        ...
        if framework and hasattr(framework, 'get_current_workspace_name'):
            try:
                current_workspace = framework.get_current_workspace_name() or 'default'
            except Exception:
                current_workspace = 'default'

        # Prefer direct DB query to avoid detached ORM instances returned by
        # some workspace_manager implementations.
        if framework and getattr(framework, 'db_manager', None):
            workspace_list = []
            with framework.db_manager.session_scope('default') as session:
            ...
            return jsonify({'workspaces': workspace_list, 'current': current_workspace})

        # Fallback: ask workspace_manager, but only touch attributes safely
        if framework and hasattr(framework, 'workspace_manager'):
            workspace_list = []
            for ws in (framework.workspace_manager.list_workspaces() or []):
                name = getattr(ws, 'name', None) or 'default'
                workspace_list.append({
                    'name': name,
                    'description': getattr(ws, 'description', '') or '',
                    'created_at': '',
                    'is_active': bool(getattr(ws, 'is_active', True)),
...
def list_docker_environments():
    """List all available Docker environments from docker_environments modules"""
    print_info("Docker environments list endpoint called")
    try:
        environments = []
        
        if framework and hasattr(framework, 'module_loader'):
            # Get all docker_environment modules using the same method as listeners
            docker_modules = framework.module_loader.get_modules_by_type("docker_environment")
            print_info(f"Found {len(docker_modules)} docker environment modules")
            
            if len(docker_modules) == 0:
                # Try to discover modules manually for debugging
                discovered = framework.module_loader._discover_modules_by_type("docker_environment")
                print_info(f"Discovered {len(discovered)} modules in docker_environments directory: {list(discovered.keys())}")
            
            for module_path, module_instance in docker_modules.items():
                try:
                    # Get module info
                    info = getattr(module_instance, '__info__', {})
                    name = info.get('name', module_path.split('/')[-1])
                    ...
                    # Get container configuration from module options/attributes
                    container_name = None
                    image_name = None
                    host_port = None
                    
                    # Try to get from module attributes (options)
                    if hasattr(module_instance, 'container_name'):
                        container_name = getattr(module_instance.container_name, 'value', None) if hasattr(module_instance.container_name, 'value') else getattr(module_instance, 'container_name', None)
                    if hasattr(module_instance, 'image_name'):
                        image_name = getattr(module_instance.image_name, 'value', None) if hasattr(module_instance.image_name, 'value') else getattr(module_instance, 'image_name', None)
                    if hasattr(module_instance, 'host_port'):
                        host_port = getattr(module_instance.host_port, 'value', None) if hasattr(module_instance.host_port, 'value') else getattr(module_instance, 'host_port', None)
                    
                    # Fallback to default values if not set
                    if not container_name and hasattr(module_instance, 'container_name'):
                        # Try to get default from OptString
                        try:
                            container_name = module_instance.container_name.default if hasattr(module_instance.container_name, 'default') else None
                        except:
                            pass
                    if not image_name and hasattr(module_instance, 'image_name'):
                        try:
                            image_name = module_instance.image_name.default if hasattr(module_instance.image_name, 'default') else None
                        except:
                            pass
                    if not host_port and hasattr(module_instance, 'host_port'):
                        try:
                            host_port = module_instance.host_port.default if hasattr(module_instance.host_port, 'default') else None
                        except:
                            pass
                    ...
                    print_error(f"Error getting info for docker environment {module_path}: {e}")
                    ...
                    environments.append({
                        ...
                        'image': None,
                        'web_port': None,
                        'display_name': module_path.split('/')[-1].replace('_', ' ').title(),
                        'error': str(e)
                    })
                    continue
        
        # Sort by name
        environments.sort(key=lambda x: x.get('display_name', x.get('name', '')))
        ...
        import traceback
        traceback.print_exc()
        # Return 200 with empty list instead of 500 to avoid 404
        return jsonify({'environments': [], 'error': str(e)})

@app.route('/api/docker_environments/<path:env_name>/start', methods=['POST'])
def start_docker_environment(env_name):
    ...
    try:
        ...
        # Find the module by name (can be module name or module path)
        docker_modules = framework.module_loader.get_modules_by_type("docker_environment")
        module_path = None
        module_instance = None
        
        # Try to find by module path first (most reliable)
        if env_name in docker_modules:
            module_path = env_name
            module_instance = docker_modules[env_name]
        else:
            # Try to find by name or partial path match
            for path, instance in docker_modules.items():
                module_name = path.split('/')[-1]
                info = getattr(instance, '__info__', {})
                display_name = info.get('name', module_name)
                # Normalize names for comparison (remove extra spaces, case insensitive)
                normalized_env = ' '.join(env_name.lower().split())
                normalized_display = ' '.join(display_name.lower().split())
                normalized_module = module_name.lower()
                
                if (normalized_env == normalized_module or 
                    normalized_env == normalized_display or
                    env_name.lower() in path.lower()):
                    module_path = path
                    module_instance = instance
                    break
        
        if not module_path or not module_instance:
        ...
        try:
            ...
            return jsonify({'success': True, 'output': output, 'result': result})
            ...
            if app.debug:
                error_msg += f"\n{traceback.format_exc()}"
            print_error(f"Error executing docker environment module: {error_msg}")
...
@app.route('/api/docker_environments/<path:env_name>/stop', methods=['POST'])
def stop_docker_environment(env_name):
    ...
    try:
        ...
        # Get container name from module
        if hasattr(module_instance, 'container_name'):
            container_name = getattr(module_instance.container_name, 'value', None) if hasattr(module_instance.container_name, 'value') else getattr(module_instance, 'container_name', None)
            if not container_name and hasattr(module_instance.container_name, 'default'):
                container_name = module_instance.container_name.default
        
        if not container_name:
            return jsonify({'success': False, 'error': f'Container name not found for environment "{env_name}". Module path: {module_path}'}), 404
...
@app.route('/api/docker_environments/<path:env_name>/logs', methods=['GET'])
...
def get_market_status():
    """Get marketplace account status"""
    try:
        if not framework:
            return jsonify({'success': False, 'error': 'Framework not initialized'}), 500
        
        # Get market command instance
        from interfaces.command_system.builtin.market_command import MarketCommand
        market_cmd = MarketCommand(framework, None, None)
        
        # Reload config to get latest state (in case it was cleared by logout)
        market_cmd._load_account_config()
        
        # Check if logged in - token must exist and not be empty
        is_logged_in = (market_cmd.token is not None and market_cmd.token != '') or \
                      (market_cmd.api_key is not None and market_cmd.api_key != '')
            ...
            try:
                ...
                if os.path.exists(config_file):
                    with open(config_file, 'r') as f:
                        config = json.load(f)
                        # Only return account info if we have a valid token
        ...
        return jsonify({
            ...
            'registry_url': market_cmd.registry_url
        })
    except Exception as e:
        print_error(f"Error getting market status: {e}")
        return jsonify({'success': False, 'error': str(e)}), 500

@app.route('/api/market/login', methods=['POST'])
def market_login():
    ...
    try:
        ...
        # Use MarketCommand to login
        from interfaces.command_system.builtin.market_command import MarketCommand
        
        market_cmd = MarketCommand(framework, None, None)
        
        response = requests.post(
            f"{market_cmd.registry_url}/api/cli/login",
            json={"email": email, "password": password},
            timeout=market_cmd.timeout
        )
        ...
        if response.status_code == 200:
            ...
            if result.get('success'):
                ...
                if token:
                    # Save config
                    market_cmd.token = token
                    market_cmd._save_account_config(token, user.get('email'), user.get('username'), expires_at)
                    ...
                    return jsonify({'success': False, 'error': 'No token received'}), 500
            else:
                return jsonify({'success': False, 'error': result.get('error', 'Unknown error')}), 400
        else:
            error_data = response.json() if response.headers.get('content-type', '').startswith('application/json') else {}
            return jsonify({'success': False, 'error': error_data.get('error', 'Login failed')}), response.status_code
            
    except requests.exceptions.ConnectionError:
        return jsonify({'success': False, 'error': 'Failed to connect to marketplace server'}), 500
    except Exception as e:
        print_error(f"Error during market login: {e}")
        return jsonify({'success': False, 'error': str(e)}), 500

@app.route('/api/market/logout', methods=['POST'])
def market_logout():
    """Logout from marketplace account"""
    try:
        if not framework:
            return jsonify({'success': False, 'error': 'Framework not initialized'}), 500
        ...
        # Clear token and save config without token
        market_cmd.token = None
        market_cmd.api_key = None
        
        # Manually clear the token from config file
        try:
            config_dir = os.path.join(os.path.expanduser("~"), ".kittysploit")
            ...
            if os.path.exists(config_file):
                ...
                # Remove token and api_key but keep other settings
                if 'token' in config:
                    del config['token']
            ...
            logging.debug(f"Could not clear config file: {e}")
        ...
        print_error(f"Error during market logout: {e}")
        return jsonify({'success': False, 'error': str(e)}), 500

@app.route('/api/market/register', methods=['POST'])
def market_register():
    """Register new marketplace account"""
    try:
        if not framework:
            return jsonify({'success': False, 'error': 'Framework not initialized'}), 500
        ...
        # Use MarketCommand to register
        from interfaces.command_system.builtin.market_command import MarketCommand
        
        market_cmd = MarketCommand(framework, None, None)
        
        response = requests.post(
            f"{market_cmd.registry_url}/api/cli/register",
        ...
        if response.status_code == 201:
            ...
            error_data = response.json() if response.headers.get('content-type', '').startswith('application/json') else {}
            return jsonify({'success': False, 'error': error_data.get('error', 'Registration failed')}), response.status_code
            
    except requests.exceptions.ConnectionError:
        return jsonify({'success': False, 'error': 'Failed to connect to marketplace server'}), 500
    except Exception as e:
        print_error(f"Error during market registration: {e}")
        return jsonify({'success': False, 'error': str(e)}), 500

@app.route('/api/market/installed', methods=['GET'])
def get_installed_modules():
    """Get list of installed marketplace modules"""
    try:
        if not framework:
            return jsonify({'success': False, 'error': 'Framework not initialized'}), 500
        ...
        # Use _get_installed_modules() instead of _list_installed_extensions()
        installed_raw = market_cmd._get_installed_modules()
        
        if not installed_raw:
            logging.debug("No installed modules found by _get_installed_modules()")
        
        # Format and enrich the installed modules with manifest data
        installed = []
        for inst in installed_raw:
            # Default values
            description = f"Installed module: {inst.get('name', '')}"
            author = 'Local'
            extension_type = inst.get('type', 'module')
            ...
            if path and os.path.exists(path):
                ...
                manifest_paths = [
                    os.path.join(path, 'extension.toml'),
                    os.path.join(path, 'latest', 'extension.toml'),
                    os.path.join(os.path.dirname(path), 'extension.toml')
                ]
                ...
                for manifest_path in manifest_paths:
                    if os.path.exists(manifest_path):
                        try:
                            ...
                            logging.debug(f"Could not parse manifest {manifest_path}: {e}")
                            continue
            
            # Normalize author (could be object or string)
            if isinstance(author, dict):
                author = author.get('username') or author.get('name') or author.get('email') or 'Local'
            elif not isinstance(author, str):
                author = str(author) if author else 'Local'
            ...
            installed.append({
                ...
                'type': extension_type,
                'installed': True,
                'marketplace_id': inst.get('marketplace_id'),
                'path': path
            })
        ...
        print_error(f"Error getting installed modules: {e}")
        import traceback
        traceback.print_exc()
        return jsonify({'success': False, 'error': str(e)}), 500
...
def list_marketplace_modules():
    ...
    try:
        ...
        # Try to get modules from marketplace API
        data = market_cmd._make_request('market/modules', params, requires_auth=False, use_new_api=True)
        ...
        if data:
            ...
            if 'extensions' in data:
                modules = data.get('extensions', [])
            elif 'modules' in data:
                modules = data.get('modules', [])
            total = data.get('total', len(modules))
            
            # Normalize author field for all modules (convert object to string)
            for module in modules:
                author = module.get('author')
                if isinstance(author, dict):
                    # Extract author name from object
                    module['author'] = author.get('username') or author.get('name') or author.get('email') or 'Unknown'
                elif not isinstance(author, str):
                    module['author'] = str(author) if author else 'Unknown'
        else:
            # Fallback: get installed modules and format them
            installed = market_cmd._get_installed_modules()
            modules = []
            for inst in installed:
                ...
                try:
                    ...
                    path = inst.get('path', '')
                    if path and os.path.exists(path):
                        manifest_path = os.path.join(path, 'extension.toml')
                        if os.path.exists(manifest_path):
                            manifest = ManifestParser.parse(manifest_path)
                            if manifest:
                                description = manifest.description or description
        ...
        print_error(f"Error listing marketplace modules: {e}")
        import traceback
        traceback.print_exc()
        return jsonify({'success': False, 'error': str(e)}), 500

@app.route('/api/market/info/<module_id>', methods=['GET'])
def get_module_info(module_id):
    ...
    try:
        ...
        # Try new API first
        module_data = market_cmd._make_request(f'market/modules/{module_id}', requires_auth=False, use_new_api=True)
        
        if not module_data:
            # Fallback to old API
            module_data = market_cmd._make_request(f'extensions/{module_id}', requires_auth=False)
        
        if module_data:
            # Normalize author field
            author = module_data.get('author')
            if isinstance(author, dict):
                module_data['author'] = author.get('username') or author.get('name') or author.get('email') or 'Unknown'
            elif not isinstance(author, str):
                module_data['author'] = str(author) if author else 'Unknown'
            
            # Ensure all fields have defaults
            if 'version' not in module_data or not module_data['version']:
                module_data['version'] = 'N/A'
            if 'description' not in module_data or not module_data['description']:
                module_data['description'] = 'No description available'
            if 'extension_type' not in module_data and 'type' not in module_data:
                module_data['type'] = 'module'
                module_data['extension_type'] = 'module'
        ...
        print_error(f"Error getting module info: {e}")
        return jsonify({'success': False, 'error': str(e)}), 500

@app.route('/api/market/install/<module_id>', methods=['POST'])
def install_module(module_id):
    ...
    try:
        ...
        # Check if logged in
        if not market_cmd.token and not market_cmd.api_key:
            return jsonify({'success': False, 'error': 'Please login first', 'requires_login': True}), 401
        
        # Create a mock args object for install
        class MockArgs:
            def __init__(self, module_id):
                self.module_id = module_id
                self.all_free = False
        
        args = MockArgs(module_id)
        success = market_cmd._install_module(args)
...
@app.route('/api/market/buy/<module_id>', methods=['POST'])
def buy_module(module_id):
    ...
    try:
        ...
        check_data = market_cmd._make_request(f'market/check/{module_id}', requires_auth=True, use_new_api=True)
        if check_data:
            has_purchased = bool(check_data.get('has_purchased', False))
            can_download = bool(check_data.get('can_download', False))
            is_author = bool(check_data.get('is_author', False))
            
            if has_purchased or can_download or is_author:
                return jsonify({
                    'success': True,
                    'message': 'You already own this module. You can install it now.',
                    'already_owned': True
                })
        
        # Make purchase request
        purchase_data = market_cmd._make_request(
            f'market/modules/{module_id}/purchase',
            method='POST',
            requires_auth=True,
            use_new_api=True
        )
        ...
        if not purchase_data:
            return jsonify({'success': False, 'error': 'Purchase failed - no response from server'}), 500
        ...
        if checkout_url:
            return jsonify({
                ...
                'message': purchase_data.get('message', 'Complete your purchase through the checkout link')
            })
        
        # If server confirms immediately
        if purchase_data.get('success'):
            return jsonify({
                'success': True,
                'message': 'Purchase successful! You can now install the module.'
            })
        ...
        print_error(f"Error purchasing module: {e}")
        import traceback
        traceback.print_exc()
        return jsonify({'success': False, 'error': str(e)}), 500

@app.route('/api/market/check/<module_id>', methods=['GET'])
def check_module_access(module_id):
    """Check if user has access to a module (purchased, free, or author)"""
    try:
        if not framework:
            return jsonify({'success': False, 'error': 'Framework not initialized'}), 500
        ...
        # Get module info and check access
        check_data = market_cmd._make_request(f'market/check/{module_id}', requires_auth=True, use_new_api=True)
        ...
        if check_data:
            ...
            currency = check_data.get('currency', 'EUR')
            
            has_access = has_purchased or can_download or is_author or (price == 0 and not requires_payment)
            ...
            # Fallback: assume requires payment if we can't check
            return jsonify({
                'success': True,
                'has_access': False,
                'requires_payment': True
        ...
        print_error(f"Error checking module access: {e}")
        return jsonify({
            'success': True,
            'has_access': False,
            'requires_payment': True
...
@app.route('/api/market/uninstall/<module_id>', methods=['POST'])
def uninstall_module(module_id):
    ...
    try:
        ...
        # Create a mock args object for uninstall
        class MockArgs:
            def __init__(self, module_id):
                self.module_id = module_id
                self.all = False
        ...
        args = MockArgs(module_id)
        success = market_cmd._uninstall_module(args)
        ...
        if success:
            ...
            return jsonify({'success': False, 'error': 'Uninstallation failed'}), 500
            
    except Exception as e:
        print_error(f"Error uninstalling module: {e}")
        import traceback
        traceback.print_exc()
        return jsonify({'success': False, 'error': str(e)}), 500
...
if SOCKETIO_AVAILABLE:
    ...
    def handle_disconnect():
        ...
        if chat_server_manager and request.sid in chat_server_manager.clients:
            client_info = chat_server_manager.clients[request.sid]
            ...
            # Disconnect client
            chat_server_manager.disconnect_client(request.sid)
            
            # Broadcast to other users
            socketio.emit('chat_user_left', {
                'username': username,
                'server_id': server_id
            }, room=server_id)
            ...
            if handler_to_use:
                ...
                def session_output_callback(text):
                    if socketio:
                        ...
                        # Support both 'text' and 'output' for compatibility
                        socketio.emit('terminal_output', {'text': text, 'output': text, 'session_id': session_id}, room=session_id)
        ...
        if command_registry:
            def run_and_prompt(cmd, args, sess_id):
                try:
                    ...
                    error_msg = f"Error: {e}\r\n"
                    if socketio:
                        socketio.emit('terminal_output', {'text': error_msg, 'output': error_msg, 'session_id': sess_id}, room=sess_id)
                finally:
                    # Print prompt after command finishes
                    prompt_msg = "\r\nkitty> "
                    if socketio:
                        socketio.emit('terminal_output', {'text': prompt_msg, 'output': prompt_msg, 'session_id': sess_id}, room=sess_id)
    ...
    def handle_terminal_exec(data):
        """Execute a command and emit completion event (for scanner and other tools)"""
        session_id = data.get('session_id')
        command = data.get('command')
        ...
        global command_registry, output_handler
        if command_registry:
            def run_and_complete(cmd, args, sess_id):
                try:
                    # Ensure terminal backend is initialized
                    ensure_terminal_backend()
                    ...
                    if handler_to_use:
                        ...
                        # Set up output callback for this session if not already set
                        global terminal_callbacks
                        if 'terminal_callbacks' not in globals():
                            terminal_callbacks = {}
                        
                        if sess_id not in terminal_callbacks:
                            def session_output_callback(text):
                                if socketio:
                                    socketio.emit('terminal_output', {'text': text, 'output': text, 'session_id': sess_id}, room=sess_id)
                            
                            terminal_callbacks[sess_id] = session_output_callback
                            handler_to_use.add_callback(sess_id, session_output_callback)
                            handler_to_use.add_callback(sess_id, session_output_callback, is_stderr=True)
                    ...
                    if socketio:
                        socketio.emit('terminal_complete', {'session_id': sess_id}, room=sess_id)
            ...
            # Run in thread with session_id
            threading.Thread(target=run_and_complete, args=(cmd, args, session_id), daemon=True).start()
        else:
            error_msg = "Error: Backend not initialized.\r\n"
            if socketio:
                socketio.emit('terminal_output', {'text': error_msg, 'output': error_msg, 'session_id': session_id}, room=session_id if session_id else '/')
                socketio.emit('terminal_complete', {'session_id': session_id}, room=session_id if session_id else '/')
    ...
    def handle_irc_connect(data):
        ...
        if irc_bridge:
            if irc_bridge.connect_user(nickname, request.sid):
                emit('irc_connected', {
                    'success': True,
                    'nickname': nickname,
    ...
    def handle_irc_message(data):
        ...
        if irc_bridge and nickname and message:
            if irc_bridge.send_message(nickname, message):
                # Message will be echoed back by IRC server
                pass
            else:
    ...
    @socketio.on('chat_create_server')
    def handle_chat_create_server(data):
        """Create a new chat server"""
        print_info(f"Received chat_create_server request: {data}")
        
        if not chat_server_manager:
            print_error("Chat server manager not available")
            emit('chat_error', {'error': 'Chat server manager not available'})
            return
        
        # Safely get and strip values, handling None cases
        server_id_raw = data.get('server_id')
        server_id = server_id_raw.strip() if server_id_raw else ''
        
        server_name_raw = data.get('server_name')
        server_name = server_name_raw.strip() if server_name_raw else ''
        
        password_raw = data.get('password')
        password = password_raw.strip() if password_raw and password_raw.strip() else None
        
        max_users = data.get('max_users', 100)
        
        if not server_id or not server_name:
            print_error(f"Invalid request: server_id={server_id}, server_name={server_name}")
            emit('chat_error', {'error': 'Server ID and name are required'})
            return
        
        print_info(f"Creating chat server: {server_id} ({server_name})")
        success, result = chat_server_manager.create_server(server_id, server_name, password, max_users)
        
        if success:
            print_success(f"Chat server '{server_name}' created successfully")
            emit('chat_server_created', {
                'server_id': server_id,
                'server_name': server_name,
                'has_password': password is not None
            })
            # Broadcast to all clients that a new server is available
            socketio.emit('chat_server_list_updated', {'action': 'created', 'server_id': server_id})
        else:
            print_error(f"Failed to create chat server: {result}")
            emit('chat_error', {'error': result})
    
    @socketio.on('chat_list_servers')
    def handle_chat_list_servers():
        """List all available chat servers"""
        if not chat_server_manager:
            emit('chat_servers_list', {'servers': []})
            return
        
        servers = chat_server_manager.list_servers()
        emit('chat_servers_list', {'servers': servers})
    ...
    def handle_chat_connect(data):
        ...
        if not server_id or not username:
            emit('chat_error', {'error': 'Server ID and username are required'})
            return
        
        success, result = chat_server_manager.connect_client(request.sid, server_id, username, password)
        if success:
            client_info = result
            server_info = chat_server_manager.get_server_info(server_id)
            message_history = chat_server_manager.get_message_history(server_id, limit=100)
            ...
            # Broadcast to other users in the server
            socketio.emit('chat_user_joined', {
                'username': username,
                'server_id': server_id
            }, room=server_id, skip_sid=request.sid)
        else:
            emit('chat_error', {'error': result})
    ...
    def handle_chat_send_message(data):
        ...
        success, result = chat_server_manager.send_message(request.sid, message)
        if success:
            msg = result
            # Broadcast to all users in the server
            server_id = chat_server_manager.clients[request.sid].server_id
    ...
    def handle_chat_disconnect():
        ...
        if request.sid in chat_server_manager.clients:
            client_info = chat_server_manager.clients[request.sid]
            server_id = client_info.server_id
            username = client_info.username
    ...
    def api_tor_status():
        ...
        try:
            if not framework:
                return jsonify({'success': False, 'error': 'Framework not available'}), 500
    ...
    def api_tor_enable():
        ...
        try:
            ...
            if result:
                ...
                return jsonify({'success': False, 'error': 'Failed to enable Tor. Make sure Tor is running.'}), 400
        except Exception as e:
    ...
    # --- TOR NETWORK MANAGEMENT (SocketIO) ---
    if SOCKETIO_AVAILABLE and socketio:
        @socketio.on('tor_connect')
        def handle_tor_connect(data):
            """Enable Tor network via SocketIO"""
            try:
                if not framework:
        ...
        def handle_tor_disconnect():
            """Disable Tor network via SocketIO"""
            try:
                if not framework:
                    emit('tor_error', {'error': 'Framework not available'})
                    return
        ...
        def handle_tor_status():
            """Get current Tor status via SocketIO"""
            try:
                if not framework:
    ...
    def handle_vnc_connect(data):
        ...
        try:
            from kittyos_cosmic.vnc_proxy import vnc_proxy
            import uuid
            
            connection_id = data.get('connection_id') or str(uuid.uuid4())
            vnc_host = data.get('host', '127.0.0.1')
            vnc_port = int(data.get('port', 5900))
            password = data.get('password', '')
            client_sid = request.sid  # Capture SID before thread creation
            
            if vnc_proxy.create_connection(connection_id, vnc_host, vnc_port, password):
                emit('vnc_connected', {'connection_id': connection_id})
                
                # Send ServerInit data immediately if available
                import base64
                server_init = vnc_proxy.get_server_init_data(connection_id)
                if server_init:
                    data_b64 = base64.b64encode(server_init).decode('utf-8')
                    emit('vnc_data', {
                        'connection_id': connection_id,
                        'data': data_b64
                    })
                
                # Start receive loop automatically
                _start_vnc_receive_loop(connection_id, client_sid)
            else:
                emit('vnc_error', {'error': 'Failed to connect to VNC server', 'connection_id': connection_id})
        except Exception as e:
            print_error(f"VNC connect error: {e}")
            emit('vnc_error', {'error': str(e), 'connection_id': connection_id})
    
    @socketio.on('vnc_send')
    def handle_vnc_send(data):
        ...
        try:
            ...
            if connection_id and data_b64:
                vnc_data = base64.b64decode(data_b64)
                client_sid = request.sid
                if vnc_proxy.send_data(connection_id, vnc_data):
                    # Start receiving loop if not already running
                    if connection_id not in _vnc_receive_threads:
                        _start_vnc_receive_loop(connection_id, client_sid)
                else:
                    emit('vnc_error', {'error': 'Failed to send data', 'connection_id': connection_id})
        except Exception as e:
    ...
    def handle_vnc_disconnect(data):
        ...
        try:
            ...
            connection_id = data.get('connection_id')
            if connection_id:
                vnc_proxy.close_connection(connection_id)
                emit('vnc_disconnected', {'connection_id': connection_id})
        except Exception as e:
            print_error(f"VNC disconnect error: {e}")
    
    _vnc_receive_threads = {}  # Track active receive threads
    
    def _start_vnc_receive_loop(connection_id, client_sid):
        """Start background thread to receive VNC data"""
        import threading
        from kittyos_cosmic.vnc_proxy import vnc_proxy
        import base64
        
        # Don't start multiple threads for same connection
        if connection_id in _vnc_receive_threads:
            return
        ...
        def receive_loop():
            try:
                while True:
                    try:
                        if not vnc_proxy.is_connected(connection_id):
                            socketio.emit('vnc_disconnected', {
                                'connection_id': connection_id
                            }, room=client_sid)
                            break
                        
                        data = vnc_proxy.receive_data(connection_id)
                        if data:
                            data_b64 = base64.b64encode(data).decode('utf-8')
                            socketio.emit('vnc_data', {
                                'connection_id': connection_id,
                                'data': data_b64
                            }, room=client_sid)
                        else:
                        ...
                        print_error(f"VNC receive loop error: {e}")
                        socketio.emit('vnc_error', {
                            'connection_id': connection_id,
                            'error': str(e)
                        }, room=client_sid)
                        break
            finally:
                # Only clear thread registry when the loop truly ends.
                _vnc_receive_threads.pop(connection_id, None)
        
        thread = threading.Thread(target=receive_loop, daemon=True)
        _vnc_receive_threads[connection_id] = thread
        thread.start()

if __name__ == '__main__':
    # Quiet HTTP stack unless KITTYCOSMIC_DEBUG=1
    import logging
    for _name in (
        "werkzeug",
        "flask",
    ...
    werkzeug_logger.disabled = not KITTYOS_WEB_DEBUG

    class _QuietWerkzeug(logging.Filter):
        def filter(self, record):
            return False

    if not KITTYOS_WEB_DEBUG:
        werkzeug_logger.addFilter(_QuietWerkzeug())

    init_framework()
    start_rpc_server()
    start_collab_server()

    main_ui = f"http://{KITTYOS_WEB_HOST}:{KITTYOS_WEB_PORT}/"
    collab_ui = f"http://127.0.0.1:{collab_server_port}/"
    print()
    print_success(f"KittyCosmic — open in browser: {main_ui}")
    if SOCKETIO_AVAILABLE and socketio:
        print_info("WebSocket (SocketIO): enabled")
    else:
        print_info("WebSocket (SocketIO): disabled — install flask-socketio for live features")
    print_info(f"KittyCollab (separate app): {collab_ui}")
    print_info("XML-RPC (framework API):    http://127.0.0.1:55553/")
    print_info("Stop the server: Ctrl+C")
    if not KITTYOS_WEB_DEBUG:
        print_info("Verbose Flask logs: set KITTYCOSMIC_DEBUG=1")
    print()

    if SOCKETIO_AVAILABLE and socketio:
        socketio.run(
            app,
            host=KITTYOS_WEB_HOST,
            port=KITTYOS_WEB_PORT,
            debug=KITTYOS_WEB_DEBUG,
            use_reloader=False,
            log_output=KITTYOS_WEB_DEBUG,
        )
    else:
        app.run(
            host=KITTYOS_WEB_HOST,
